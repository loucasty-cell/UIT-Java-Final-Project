...
test.describe('Session Booking & Management', () => {
  ...
  test('Can navigate to mentors page', async ({ page }) => {
    const mentorsLink = page.locator('a:has-text("Mentor"), a:has-text("Find Mentor"), a:has-text("Browse")').first();
  ...
  test('Upcoming sessions display on dashboard', async ({ page }) => {
    const sessionsSection = page.locator('text=Continue Learning, text=Upcoming, text=Sessions').first();
  ...
  test('Session details can be viewed', async ({ page }) => {
    const viewDetailsBtn = page.locator('button:has-text("View Details"), a:has-text("Details")').first();
