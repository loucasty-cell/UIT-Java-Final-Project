...
function NoticeboardPage() {
  ...
  const needsQuery = useQuery({
    queryKey: ["learning-needs"],
    queryFn: () => learningNeedsService.list(),
    staleTime: 30000,
    refetchInterval: 60000,
  });

  const teachSkillsQuery = useQuery({
    queryKey: ["teach-skills"],
    queryFn: () => skillsService.getUserSkills("TEACH"),
    staleTime: 60000,
  });

  const loading = needsQuery.isLoading || teachSkillsQuery.isLoading;
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Ask for a skill that is not yet being offered in Find Mentors. Teachers with that skill
      ...
      <div className="grid gap-3 rounded-xl border bg-primary/5 p-4 text-sm sm:grid-cols-3">
        <p className="flex items-center gap-2">
          <BookOpen className="h-4 w-4 text-primary" /> Learners post an unmet skill need.
        </p>
        <p className="flex items-center gap-2">
          <Lightbulb className="h-4 w-4 text-primary" /> Teachers offer help for skills they teach.
        </p>
        <p className="flex items-center gap-2">
          <Users className="h-4 w-4 text-primary" /> The learner receives a notification.
        </p>
      </div>
      ...
      {error && (
        <p role="alert" className="text-destructive">
          {error instanceof Error ? error.message : "Unable to load learning needs."}
        </p>
      )}
      {!loading && !error && !filteredNeeds.length && (
        <Card>
          <CardContent className="p-8 text-center">
            No learning needs match yet. Post the first one.
          </CardContent>
        </Card>
      )}
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 text-sm">
                  <Badge variant="secondary" className="gap-1">
                    <Clock3 className="h-3.5 w-3.5" /> {need.durationMinutes} minutes
                  </Badge>
                  {need.availabilityText && (
                    <Badge variant="outline">{need.availabilityText}</Badge>
                  )}
                  {(need.allowedModes?.length ? need.allowedModes : ["VOLUNTEER"]).map((mode) => (
                    <Badge key={mode} variant="outline">
                      {modeLabel(mode as NoticeboardMode)}
                    </Badge>
                  ))}
                  {need.exchangeSkillName && (
                    <Badge variant="outline">Exchange: {need.exchangeSkillName}</Badge>
                  )}
                  <Badge variant="outline">
                    {need.offerCount} teacher offer{need.offerCount === 1 ? "" : "s"}
                  </Badge>
                </div>
                <div className="flex flex-wrap gap-2">
                  {mine ? (
                    <Button variant="outline" onClick={() => void remove(need)}>
                      <Trash2 className="mr-2 h-4 w-4" /> Remove notice
                    </Button>
                  ) : (
                    <Button
      ...
      <CreateNeed
        ...
        reload={reload}
      />
      <OfferToTeach need={offering} onClose={() => setOffering(null)} reload={reload} />
    </div>
  );
}
...
function CreateNeed({
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success("Learning need posted", {
        description: "Teachers who have this skill can now offer to teach.",
      });
      setTitle("");
      setSkillName("");
      setDescription("");
    ...
    <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Use this board when you cannot find an existing teaching post for the skill.
          </DialogDescription>
        </DialogHeader>
        <div>
          <Label htmlFor="need-title">Learning goal</Label>
          <Input
            id="need-title"
            value={title}
            onChange={(event) => setTitle(event.target.value)}
            maxLength={150}
            placeholder="Example: Learn Figma for my first UI project"
          />
        </div>
        <div>
          <Label htmlFor="need-skill">Skill to learn</Label>
          <Input
            id="need-skill"
            value={skillName}
          ...
          />
          <p className="text-xs text-muted-foreground">
            New skills are added to your learning portfolio.
          </p>
        </div>
        <div>
          <Label htmlFor="need-description">What help do you need?</Label>
          <Textarea
            id="need-description"
            value={description}
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Teachers can choose only from these modes.
          </p>
          <div className="grid gap-2 sm:grid-cols-3">
            {(["POINTS", "SKILL_SWAP", "VOLUNTEER"] as NoticeboardMode[]).map((mode) => (
              <label
                key={mode}
        ...
        {modes.includes("SKILL_SWAP") && (
          <div>
            <Label htmlFor="exchange-skill">Your skill to exchange</Label>
            <select
              id="exchange-skill"
              className="h-10 w-full rounded-md border bg-background px-3"
              value={exchangeUserSkillId}
              onChange={(event) => setExchangeUserSkillId(event.target.value)}
            >
            ...
            {!teachSkills.length && (
              <p className="text-xs text-destructive">
                Add a teaching skill in Settings before offering skill exchange.
              </p>
            )}
          </div>
        )}
...
function OfferToTeach({
  need,
  onClose,
  reload,
}: {
  need: LearningNeedResponse | null;
  onClose: () => void;
  reload: (silent?: boolean) => Promise<void>;
}) {
  ...
  const allowedModes = (
    need?.allowedModes?.length ? need.allowedModes : ["VOLUNTEER"]
  ) as NoticeboardMode[];
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success("Teaching offer sent", {
        description: `${need.learnerName} has been notified with your proposed time.`,
      });
      setMessage("");
      setDate("");
      setTime("");
  ...
  return (
    <Dialog open={!!need} onOpenChange={(value) => !value && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Offer to teach {need?.skillName}</DialogTitle>
          <DialogDescription>
            Choose a time that fits the learner's availability for this {need?.durationMinutes}
            -minute learning need.
          </DialogDescription>
        </DialogHeader>
        ...
        <div>
          ...
          <select
            ...
            className="h-10 w-full rounded-md border bg-background px-3"
            value={mode}
            onChange={(event) => setMode(event.target.value as NoticeboardMode)}
          >
          ...
          {mode === "SKILL_SWAP" && (
            <p className="mt-1 text-xs text-muted-foreground">
              Learner offers: {need?.exchangeSkillName || "a teaching skill"}.
            </p>
          )}
        </div>
        ...
        <div>
          <Label htmlFor="teaching-offer-message">Message (optional)</Label>
          <Textarea
            id="teaching-offer-message"
            value={message}
            onChange={(event) => setMessage(event.target.value)}
            maxLength={2000}
            placeholder="Example: I can guide you through the basics and practise with you."
          />
        </div>
        {error && (
          <p role="alert" className="text-sm text-destructive">
