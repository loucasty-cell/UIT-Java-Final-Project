...
function WalletPage() {
  ...
  if (enabled && (balance.isPending || transactions.isPending))
    return (
      <p role="status" className="p-8">
        Loading wallet…
      </p>
    );
  if (balance.error || transactions.error)
    return (
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage your SkillBridge points and activity.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {!enabled && (
        <div className="flex items-center justify-between gap-4 rounded-lg border border-dashed border-primary/30 bg-primary/5 p-4 text-sm text-muted-foreground">
          ...
          <Button asChild size="sm">
            <Link to="/login" search={{ redirect: "/wallet" }}>
              Sign in
            </Link>
          </Button>
        </div>
        ...
        <Card className="md:col-span-2 border-primary/20 bg-card shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-foreground">
              <WalletIcon className="h-5 w-5 text-primary" /> Available points
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-4xl font-bold text-primary">{points.availablePoints}</div>
            <p className="mt-1 text-muted-foreground">
              Total: {points.totalEarned - points.totalSpent} points
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Coins className="h-5 w-5 text-primary" /> Held in escrow
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{points.heldPoints}</div>
            <p className="text-sm text-muted-foreground">Released when sessions complete</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-0">
          {transactions.isLoading ? (
            ...
            <div className="divide-y">
              {rows.map((tx: WalletTransactionResponse) => {
                const amount = tx.availableDelta ?? tx.amount ?? 0;
                const positive = amount >= 0;
                return (
                  <div key={tx.id} className="flex items-center justify-between gap-4 p-4">
                    <div className="flex min-w-0 items-center gap-3">
                      <div
                        className={`rounded-full p-2 ${positive ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}
                      >
                        {positive ? (
